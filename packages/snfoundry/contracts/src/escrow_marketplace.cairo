...
pub mod EscrowMarketplace {
    ...
    use crate::types::{
        Dispute, Product, ProductStatus, ProductTrait, Shipment, ShipmentStatus, ShipmentTrait,
    };
    ...
    // Imagine a buyer pays for a product, there should be a time limit within which the seller
    // should confirm_shipped, else the contract returns the buyer's money
    ...
    pub impl EscrowMarketplaceImpl of IEscrowMarketPlace<ContractState> {
        ...
        fn add_products_to_group(
            ref self: ContractState, group_id: u256, number_of_products: u256,
        ) {
        ...
        fn remove_product_group(ref self: ContractState, group_id: u256) {
            ...
            let default_product = ProductTrait::default();
            self.id_to_product.entry(group_id).write(default_product);

            self.emit(ProductRemoved { product_id: group_id, timestamp: get_block_timestamp() });
        }
        ...
        fn buy_product(ref self: ContractState, group_id: u256, number_to_purchase: u256) {
            ...
            let mut product = self.id_to_product.entry(group_id).read();
            assert(product != ProductTrait::default(), 'Invalid Product');
            ...
            let token_dispatcher = IERC20Dispatcher {
                contract_address: self.accepted_token.read(),
            };
            ...
            self
                .emit(
                    ProductPurchased {
                        product_id: group_id, buyer, timestamp: get_block_timestamp(),
                    },
                );
        }

        fn confirm_shipped(
            ref self: ContractState, shipment_id: u256, expected_shipment_time: u64,
        ) {
            let caller = get_caller_address();
            ...
            self.emit(ProductShipped { shipment_id, timestamp: get_block_timestamp() });
        }
        ...
        fn confirm_received(ref self: ContractState, shipment_id: u256) {
            ...
            self.shipments.entry(shipment_id).write(shipment);
            self.emit(ShipmentReceived { shipment_id, timestamp: get_block_timestamp() });
        }
        ...
        fn claim_funds(ref self: ContractState, shipment_id: u256) {
            ...
            self.emit(FundsClaimed { shipment_id, amount });
        }
        ...
        fn get_products_list(self: @ContractState) -> Array<Product> {
            let mut products_array: Array<Product> = ArrayTrait::new();

            for i in 1..self.next_product_id.read() {
                let current_product = self.id_to_product.entry(i).read();
        ...
        fn get_product(self: @ContractState, group_id: u256) -> Product {
            let product = self.id_to_product.entry(group_id).read();

            if product.group_id != 0 {
                product
            } else {
                ProductTrait::default()
            }
        }

        fn get_shipment(self: @ContractState, shipment_id: u256) -> Shipment {
            let shipment = self.shipments.entry(shipment_id).read();

            if shipment.shipment_id != 0 {
                shipment
            } else {
                ShipmentTrait::default()
            }
        }

        fn get_seller_products(self: @ContractState, seller: ContractAddress) -> Array<Product> {
            let mut seller_products_array = ArrayTrait::new();

            let next_product_id = self.next_product_id.read();

            for i in 1..next_product_id {
                let current_product = self.id_to_product.entry(i).read();

                if current_product.seller == seller {
                    seller_products_array.append(current_product);
                }
            }

            seller_products_array
        }
    }
}
